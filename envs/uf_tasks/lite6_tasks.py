...
from ..uf_robots.custom_lite6 import Lite6
...
register(
    id="Lite6Reach-v1",
    entry_point="envs.uf_tasks.lite6_tasks:Lite6ReachEnv",
    max_episode_steps=50,
)

class Lite6FlipEnv(RobotTaskEnv):
    """Flip task wih Lite6 robot.
    ...
    def __init__(
        ...
        robot = Lite6(sim, block_gripper=False, base_position=np.array([-0.5, 0.0, 0.0]), control_type=control_type)
...
class Lite6PickAndPlaceEnv(RobotTaskEnv):
    """Pick and Place task wih Lite6 robot.
...
class Lite6PushEnv(RobotTaskEnv):
    """Push task wih Lite6 robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Lite6(sim, block_gripper=True, base_position=np.array([-0.5, 0.0, 0.0]), control_type=control_type)
...
class Lite6ReachEnv(RobotTaskEnv):
    """Reach task wih Lite6 robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Lite6(sim, block_gripper=True, base_position=np.array([-0.5, 0.0, 0.0]), control_type=control_type)   ## changed base position x coordinate from -0.5 to 0
...
class Lite6SlideEnv(RobotTaskEnv):
    """Slide task wih Lite6 robot.
...
class Lite6StackEnv(RobotTaskEnv):
    """Stack task wih Lite6 robot.
