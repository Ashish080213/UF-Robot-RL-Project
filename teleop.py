...
print("Press keys 1 to 6 to select a joint.")
print("Use UP/DOWN arrows to move the selected joint.")

while True:
    keys = p.getKeyboardEvents()

    # Detect joint selection keys (1-6)
    for k in keys:
        if keys[k] & p.KEY_WAS_TRIGGERED:
            if k in [ord(str(i)) for i in range(1, 7)]:
                selected_joint = k - ord('1')
    ...
    if selected_joint is not None:
        if p.B3G_UP_ARROW in keys and keys[p.B3G_UP_ARROW] & p.KEY_IS_DOWN:
            ...
            print(f"Joint {selected_joint + 1} angle increased to {current_angles[selected_joint]:.2f}")
        if p.B3G_DOWN_ARROW in keys and keys[p.B3G_DOWN_ARROW] & p.KEY_IS_DOWN:
            current_angles[selected_joint] -= increment
            print(f"Joint {selected_joint + 1} angle decreased to {current_angles[selected_joint]:.2f}")
